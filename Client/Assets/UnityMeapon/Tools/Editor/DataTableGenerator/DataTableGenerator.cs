...
namespace MeaponUnity.Editor
{
    public sealed class DataTableGenerator
    {
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "Dream.DataTable");
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                {
                    string typeName = dataTableProcessor.GetType(i).Name.Replace("`", "_");
                    stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), typeName).AppendLine();
                }
            }
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                {
                    string typeName = dataTableProcessor.GetType(i).Name.Replace("`", "_");
                    stringBuilder.AppendFormat("                    {0} = binaryReader.Read{1}();", dataTableProcessor.GetName(i), typeName).AppendLine();
                }
            }
